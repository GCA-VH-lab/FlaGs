...
def spLocal(faa,acc): #getting species name using assembly number or accession
	if getSpeciesFromGCF(faa)=='Nothing':
		faaFile=faa+'.faa.gz'
...
def sortGCFvsGCA(gcagcfSet):
	if gcagcfSet!={'N', 'I', 'A'}:
		...
		for items in gcagcfSet:
			...
			if items[2]=='F':
				Fset.add(items)
		if len(Aset)>0 and len(Fset)>0:
			prioritySet=set()
			for gcas in Aset:
				for gcfs in Fset:
					if gcas.split('_')[1]==gcfs.split('_')[1]:
						prioritySet.add(gcfs)
					if gcas.split('_')[1]!=gcfs.split('_')[1]:
						prioritySet.add(gcfs)
			if len(prioritySet)==0:
				for gcaall in Aset:
					prioritySet.add(gcaall)
			if len(prioritySet)!=0:
				return prioritySet
			else:
				return gcagcfSet
...
if not args.localGenomeList:
	with open (args.out_prefix+'_NameError.txt', 'w') as fbad:
		for query in queryList:
			q+=1
			if len(query)<2:
				if query[0][:2]=='WP' and query[0][-2]=='.': #WP Accession
					queryDict[query[0]+'#'+str(q)]=accession_from_wp(query[0])
				elif query[0][:2]=='XP' and query[0][-2]=='.': #XP Accession
					assemList=[]
				...
				else: #other accessions
					if identicalProtID(query[0])!=query[0]:
						if accession_from_wp(identicalProtID(query[0])):
							asset=set()
							for elements in accession_from_wp(identicalProtID(query[0])):
								asset.add(elements)
							if len(asset)>0:
								queryDict[identicalProtID(query[0])+'#'+str(q)+'.'+query[0]]=sortGCFvsGCA(asset)
					elif identicalProtID(query[0])==query[0]:
						if accession_from_wp(query[0]):
							asset=set()
							for elements in accession_from_wp(query[0]):
								asset.add(elements)
							if len(asset)>0:
								queryDict[query[0]+'#'+str(q)]=sortGCFvsGCA(asset)
					else:
						ne+=1
						print(query[0], file= fbad)
			else:
				if query[0][:2]=='XP' and query[0][-2]=='.': #XP Accession
					asset=set()
					for bioprojs in accession_from_xp(query[0]):
						if bioprojs in bioDict:
					...
					if len(asset)>0:
						queryDict[query[0]+'#'+str(q)]=sortGCFvsGCA(asset)
				elif query[0][:2]!='XP' and query[0][-2]=='.': #not XP Accession
					asset=set()
					if accession_from_wp(query[0]):
						for elements in accession_from_wp(query[0]):
							if query[1]==elements:
					...
					if identicalProtID(query[0]):
						if identicalProtID(query[0])[:2]!='XP': #not XP Accession
							asset=set()
							if accession_from_wp(identicalProtID(query[0])):
								for elements in accession_from_wp(identicalProtID(query[0])):
									if query[1]==elements:
										asset.add(query[1])
							queryDict[identicalProtID(query[0])+'#'+str(q)+'.'+query[0]]=sortGCFvsGCA(asset)
						if identicalProtID(query[0])[:2]=='XP': # XP Accession
							asset=set()
							for bioprojs in accession_from_xp(identicalProtID(query[0])):
								if bioprojs in bioDict:
									if bioDict[bioprojs]==query[1]:
										asset.add(query[1])
							if len(asset)>0:
					...
					for elements in files:
						if '.gff.gz' in elements: #Check if GFF.gz is there
							...
							try:
								...
								gffFileName='./'+assemblyName[item]+'.gff.gz'
								gffFileDownloaded=open(assemblyName[item]+'.gff.gz', 'wb')
								...
								if os.path.isfile(gffFileName):
									if os.path.getsize(gffFileName)==gffFileSize:
												...
												if genes==geneList[0]:
													if query.split('#')[0]==geneProt[genes]:
														for line in LineList:
															if genes==line[1]:
																FoundDict[query]='Yes'
																speciesDict[query]=getSpeciesFromGCF(assemblyName[item])#accnr_list_dict[item].split('\t')[0]
										...
										time.sleep(1)
										print('Genome Assembly file was not downloaded completely')
										FlankFoundDict[query]='No'
					...
					if not args.localGenomeList:
						try:
							...
							if record.id==queries.split('#')[0]:
								record.id= queries+'|'+remBadChar(speciesDict[queries])
								record.description= ''
						...
						for item in NqueryDict[queries]:
							...
							querySeqDict[queries+'|'+remBadChar(spLocal(item,queries.split('#')[0]))]=str(seqLocal(item,queries.split('#')[0]))
...
if not args.tree_order:
	...
	for eg in egs:
		if eg!='':
			...
			text = canvas.create_text(textspace/2,line_pos_y, text=org, fill="#404040", font=myFont12)
