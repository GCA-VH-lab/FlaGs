...
def seqFasLocal(faa,acc): #making fasta file from accession
	...
	for record in SeqIO.parse(fastaSeq, "fasta"):
		if record.id==acc.split('#')[0]:
			if spName=='Nothing':
				record.id=acc+'|'+remBadChar(record.description.split('[')[-1][:-1]).replace(' ','_')#.replace(':','_').replace('[','_').replace(']','_')
			else:
				record.id=acc+'|'+remBadChar(spName)
			record.description=''
				...
				if query[0][:2]=='WP' and query[0][-2]=='.': #WP Accession
					...
					if identicalProtID(query[0])!=query[0]:
						if accession_from_wp(identicalProtID(query[0])):
							if accession_from_wp(identicalProtID(query[0]))!='NAI':
								queryDict[identicalProtID(query[0])+'#'+str(q)+'.'+query[0]]=accession_from_wp(identicalProtID(query[0]))
					elif identicalProtID(query[0])==query[0]:
						if accession_from_wp(query[0]):
							if accession_from_wp(query[0])!='NAI':
								queryDict[query[0]+'#'+str(q)]=accession_from_wp(query[0])
					else:
						ne+=1
						...
						if identicalProtID(query[0])[:2]!='XP': #not XP Accession
							...
							if accession_from_wp(identicalProtID(query[0])):
								for elements in accession_from_wp(identicalProtID(query[0])):
									if query[1]==elements:
										asset.add(query[1])
							queryDict[identicalProtID(query[0])+'#'+str(q)+'.'+query[0]]=asset
				...
				if FlankFoundDict[queries]=='Yes':
					if not args.localGenomeList:
						...
						if record.id==queries.split('#')[0]:
							record.id= queries+'|'+speciesDict[queries].replace(' ','_').replace(':','_').replace('[','_').replace(']','_')
							record.description= ''
						...
						for item in NqueryDict[queries]:
							...
							querySeqDict[queries+'|'+spLocal(item,queries.split('#')[0])]=str(seqLocal(item,queries.split('#')[0]))
		...
		for queries in accFlankDict:
			for items in sorted(accFlankDict[queries]):
				if queryStrand[queries]=='+':
					...
					ids=accFlankDict[queries][items][:-1]
					lengths=LengthDict[accFlankDict[queries][items][:-1]]
					species=queries+'|'+remBadChar(speciesDict[queries])#.replace(' ','_').replace(':','_').replace('[','_').replace(']','_')
					qStrand=queryStrand[queries]
					nStrand=accFlankDict[queries][items][-1]
					family=familyDict[accFlankDict[queries][items][:-1].split('#')[0]]
